...
"venv\Scripts\python.exe" -c "import socket,sys; s=socket.socket(); s.settimeout(.3); sys.exit(1 if s.connect_ex(('127.0.0.1', %BACKEND_PORT%)) == 0 else 0)"
if errorlevel 1 (
    echo [错误] 端口 %BACKEND_PORT% 已被占用。请关闭占用该端口的程序后重试。
    goto :failed
)

"venv\Scripts\python.exe" -c "import socket,sys; s=socket.socket(); s.settimeout(.3); sys.exit(1 if s.connect_ex(('127.0.0.1', %FRONTEND_PORT%)) == 0 else 0)"
if errorlevel 1 (
    echo [错误] 前端开发端口 %FRONTEND_PORT% 已被占用。请关闭占用该端口的程序后重试。
    goto :failed
)
...
powershell -NoLogo -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "  Start-Sleep -Seconds 2; $npm=(Get-Command npm.cmd).Source; $frontend=Start-Process -FilePath $npm -ArgumentList @('run','dev') -WorkingDirectory (Join-Path $root 'frontend') -WindowStyle Hidden -PassThru;" ^
  "  Start-Sleep -Seconds 3; if ($frontend.HasExited) { throw '前端进程启动失败，请检查上方 npm 输出。' };" ^
  "  Write-Host ''; Write-Host '==================================================' -ForegroundColor Cyan; Write-Host ' 启动成功：http://localhost:%BACKEND_PORT%' -ForegroundColor Green; Write-Host ' 当前部分业务模块尚未编码完成，部分接口返回 404。' -ForegroundColor Yellow; Write-Host ' 关闭所有前端页面后，前后端服务会自动退出。' -ForegroundColor Cyan; Write-Host '==================================================' -ForegroundColor Cyan; Write-Host '';" ^
