...
前置依赖：Python 3.10+、Node.js（包含 npm）。一键脚本会自动创建根目录下的 `venv`、安装后端和前端依赖、启动 FastAPI 与 Vue 3，并默认打开 [http://localhost:8082](http://localhost:8082)。
...
首次运行如果根目录没有 `.env`，脚本会提示并自动复制 `.env.example`；正式使用前请修改其中的 `JWT_SECRET`。本地脚本以 `8082`（后端）和 `5173`（Vite 前端）为默认端口；如果端口被占用，会从对应默认值开始逐个顺延并选择首个可用端口，控制台会打印实际访问地址和前端端口。只有候选端口耗尽或服务启动失败时才退出并给出中文错误信息。关闭所有由脚本打开的前端页面后，页面心跳会在短暂宽限期内失效，脚本随后自动关闭前端和后端进程并释放实际使用的端口；普通刷新不会误关服务。
...
默认不依赖外部 LLM，适合本地演示；私有化 Qwen2.5-7B 运行时主要成本是模型显存。SQLite、合同文件和报告均落在 `data_storage/`，向量库目录为 `chroma_db/`。可使用 `docker compose up --build` 固定暴露宿主机 `8082` 端口（Docker 映射不参与一键脚本的自动顺延；若宿主机端口冲突，请调整 Compose 映射）。完整月度区间和测算公式见 [成本测算](docs/COST_ESTIMATE.md)，演示环境和推荐演示路径见 [演示环境](docs/DEMO_ENV.md)。
